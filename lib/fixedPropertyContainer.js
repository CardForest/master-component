...
FixedPropertyContainer.prototype.$$serializeProperty = function (propertyKey, config) {
  if (config.$primitive) {
    return this.$raw[propertyKey];
  } else if (('$serialize' in this.$raw[propertyKey])) {
    return this.$raw[propertyKey].$serialize();
  } // else functions or non-serializable component
};
Object.defineProperty(FixedPropertyContainer.prototype, '$$serializeProperty', {enumerable: false});
