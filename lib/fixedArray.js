...
FixedArray.prototype.$snapshot = function (scope) {
  var res = [];
  for (var i = 0; i < this.$config.$length; i++) {
    var serialized = this.$$snapshotProperty(i, this.$config.$elem, scope);
    if (serialized == null) {
      // it doesn't really make sense for some elements of the array to not be serializable
      throw Error('element of an array must be serializable');
    }
    res.push(serialized);
  }
  return res;
